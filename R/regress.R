...
######################################################## SC
    ...
    if ("aov" %in% sum_check) {
        atab <- anova(object$model)
        atab[, "Pr(>F)"] = format.pval(atab[, "Pr(>F)"], eps = 0.001, 
            digits = 4)
        cat("ANOVA Table:\n")
        format(atab, scientific = FALSE) %>% print
        cat("\n")
    }
    if ("pwise" %in% sum_check) {
        aFit <- aov(object$model)
        mt <- suppressWarnings(model.tables(aFit, "means"))
        cat("  Tukey multiple comparisons of means\n")
        if (is.null(mt$n)) 
            cat("****No factor found****\n\n")
        else {
            aTHSD <- TukeyHSD(aFit)
            cat("    ", format(100 * attr(aTHSD, "conf.level"), 
                2), "% family-wise confidence level\n", sep = "")
            for (i in 1:length(aTHSD)) {
                cat("\n", names(aTHSD)[i], ":\n")
                atab <- aTHSD[[i]]
                atab[, 1:3] <- round(atab[, 1:3], 3)
                atab <- as.data.frame(atab)
                atab[, 4] <- format.pval(atab[, 4], eps = 0.001, 
                  digits = 3)
                format(atab, scientific = FALSE) %>% print
                cat("\n")
            }
